...
namespace Traveless
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void FlightsSearchButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            int from = this.SearchCodefrom.SelectedIndex;
            Airport fromAirport = AirportManager.Airports[from];
            string fromAirportCode = fromAirport.Code;
            // Get from airport code that user selected
            int to = this.SearchCodeTo.SelectedIndex;
            Airport toAirport = AirportManager.Airports[to];
            string toAirportCode = toAirport.Code;
            // Get to airport code that user selected
            string weekday = SeachDay.Text;
        ...
        private void PopulateFlightInputs(FlightViewModel? fvw)
        {
            if (fvw != null)
            {
                ...
                dayNAme.Text = fvw.WeekDay;
                // Set Flight tab "Time" input to selected time
                timeInFlight.Text = fvw.Time;
                ...
                costInFlight.Text = CostPerSeatFormatted;
            }
        }
        ...
        private void FlightsReserveButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (flight != null)
            {
                ...
                string nameFlight = nameInFlight.Text;
                // Get Flight tab "Citizenship" input value
                string CitizenshipFlight = citizenshipinFlight.Text;
                // Call ReserveFlight with flight, name, and citizenship
                ReserveFlight(flight, nameFlight, CitizenshipFlight);
        ...
        private void ReservationsSearchButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            string searchCode = ReserveCode.Text;
            // Get airline code search input value
            string airlineCode = AirlineCodeInReserve.Text;
            // Get name search input value
            string nameSearch = NameInReserve.Text;
            // Call SearchReservations with reservation code, airline code, and name
            SearchReservations(searchCode,airlineCode,nameSearch);
        }
        ...
        private void PopulateReserveInputs(ReservationViewModel? rvm)
        {
            if (rvm != null)
            {
                ...
                ReservationInputCodeTop.Text = rvm.Reservation.Code;
                // Set "Flight" input value to flight code
                ReservationFlightTop.Text = rvm.Reservation.Flight.Code;
                ...
                ReservationAirlineTop.Text = airline.Name;
                ...
                ResrvationCostTops.Text = costPerSeatFormatted.ToString();

                // Set "Name" input value to name on reservation
                ReservationNameeeTop.Text = rvm.Name;
                // Set "Citizenship" input value to citizenship on reservation
                ReservationCitizenshipTop.Text = rvm.Citizenship;
                // Check if reservation is active
                if (rvm.Reservation.IsActive)
                {
                    ReservationStatusBox.Text = "Active";
                }
                // Select "Active" option in "Status" combobox
                // Otherwise
                else
                {
                    ReservationStatusBox.Text = "Inactive";
                }
        ...
        private void ReservationsUpdateButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            string reservationCodeUpdate = ReservationInputCodeTop.Text;
            // Get "Name" input value
            string NameUpdate = ReservationNameeeTop.Text;
            // Get "Citizenship" input value
            string CitizenshipUpdate = ReservationCitizenshipTop.Text;
            // Create bool variable called "isReservationActive"
            bool isReservationActive;
            // Check if "Status" combobo has "Active" selected
            if (ReservationStatusBox.Text == "Inactive")
            {
            ...
            UpdateReservation(reservationCodeUpdate,NameUpdate,CitizenshipUpdate,isReservationActive);
        }
        ...
        private void PopulateReservations(string? code, string? airlineCode, string? name)
        {
            ...
            foreach (var reservation in ReservationManager.Reservations)
            {
                ...
                if ((code == null && airlineCode == null && name == null ? true : false) || (reservation.Code.Contains(code) && reservation.Flight.AirlineCode.Contains(airlineCode) && reservation.Name.Contains(name)))
                {
